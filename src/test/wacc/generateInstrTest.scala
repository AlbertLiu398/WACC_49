...
import Shift._
  ...
  class generateInstrTest extends AnyFlatSpec with Matchers {
    ...
    it should "generate Add instruction" in {
      ...
      refreshAndGenerate(ast) shouldBe List(I_Move(Reg(8, 64), ImmVal(1)), I_Move(Reg(9, 32), Reg(8, 32)), I_Move(Reg(8, 64), ImmVal(1)), I_Add(Reg(8, 32), Reg(9, 32), Reg(8, 32), true), I_Branch(I_Label("_errOverflow"), VS), I_Sxtw(Reg(8, 64), Reg(8, 32)))
      codeGenerator.revertTempRegs()
    }

    it should "generate Sub instruction easy" in {
      val ast = Sub(IntLiter(1), IntLiter(2))
      refreshAndGenerate(ast) shouldBe List(I_Move(Reg(8, 64), ImmVal(1)), I_Move(Reg(9, 32), Reg(8, 32)), I_Move(Reg(8, 64), ImmVal(2)), I_Sub(Reg(8, 32), Reg(9, 32), Reg(8, 32), true), I_Branch(I_Label("_errOverflow"), VS), I_Sxtw(Reg(8, 64), Reg(8, 32)))
      codeGenerator.revertTempRegs()
    }
    ...
    it should "generate Sub instruction" in {
      ...
      refreshAndGenerate(ast) shouldBe  List(I_Move(Reg(8, 64), ImmVal(1)), I_Move(Reg(9, 64), Reg(8, 64)), I_Move(Reg(8, 64), ImmVal(2)), I_Move(Reg(10, 32), Reg(8, 32)), I_Move(Reg(8, 64), ImmVal(1)), I_Sub(Reg(8, 32), Reg(10, 32), Reg(8, 32), true), I_Branch(I_Label("_errOverflow"), VS), I_Sxtw(Reg(8, 64), Reg(8, 32)), I_Sub(Reg(8, 32), Reg(9, 32), Reg(8, 32), true), I_Branch(I_Label("_errOverflow"), VS), I_Sxtw(Reg(8, 64), Reg(8, 32)))
      codeGenerator.revertTempRegs()
    }
